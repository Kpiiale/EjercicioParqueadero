...
// -----------------------------------------------------------------
    ...
    public double darTiempoPromedio() {
        ...
        int totalTiempo = 0;
        int carrosEnParqueadero = 0;
        for (Puesto puesto : puestos) {
            if (puesto.darCarro()!= null) {
                totalTiempo += (21 - puesto.darCarro().darHoraLlegada()); // Hora de cierre - Hora de entrada
                carrosEnParqueadero++;
            }
        }

        return (double) totalTiempo / carrosEnParqueadero;
    }

    //2.2

    public Carro darCarroMasDeOchoHoras() {
        Carro carroMasLargo = null;
        int maxHoras = 0;

        for (Puesto puesto : puestos) {
            if (puesto.darCarro() != null) {
                int horasEnParqueadero = puesto.darCarro().darTiempoEnParqueadero(horaActual);

                if (horasEnParqueadero > 8 && horasEnParqueadero > maxHoras) {
                    maxHoras = horasEnParqueadero;
                    carroMasLargo = puesto.darCarro();
                }
            }
        }

        return carroMasLargo;
    }
    ...
    public int contarCarrosQueComienzanConPlacaPB() {
        int count = 0;
        for (Puesto puesto : puestos) {
            if (puesto.darCarro() != null && puesto.darCarro().darPlaca().startsWith("PB")) {
                count++;
            }
        }
        return count;
